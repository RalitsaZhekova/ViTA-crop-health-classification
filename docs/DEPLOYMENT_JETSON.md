...
The VITA-owned `runtime/payload` and `runtime/engines` bind mounts use Docker's private `:Z` SELinux relabeling so the read-only container can write only to those two explicitly mounted directories. Before model loading, deployment creates and removes a probe file in each mount. This relabeling is scoped to paths below `/data/code/VITA`; it does not modify another team's directory or container.
