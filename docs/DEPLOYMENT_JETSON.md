...
The payload image extends `nvcr.io/nvidia/pytorch:25.01-py3-igpu`, matching the stack verified inside the target Orin's NVIDIA runtime: NumPy 1.26.4, PyTorch `2.6.0a0+ecf3bae40a.nv25.01`, CUDA 12.8, TensorRT 10.8.0.40, and Torch-TensorRT 2.6.0a0. This PyTorch build and the image's OpenCV binaries use the NumPy 1.x ABI, so the payload retains NumPy 1.26.4. TerraTorch 1.1.1 supports that ABI and pins segmentation-models-pytorch 0.5.0, which also satisfies OmniCloudMask 1.7.1. The real 95,484,420-parameter Prithvi checkpoint was strict-loaded against that TerraTorch release with every key matched. The build runs `pip check`, imports NumPy, Rasterio, SciPy, OpenCV, TerraTorch, OmniCloudMask, PyTorch, TensorRT, and Torch-TensorRT together, and asserts every pinned accelerated-stack version, failing if dependency resolution disturbs the NVIDIA framework stack.
